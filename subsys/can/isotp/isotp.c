...
#define LOG_LEVEL CONFIG_CAN_ISOTP_LOG_LEVEL
#include <logging/log.h>
LOG_MODULE_REGISTER(isotp);

#ifdef CONFIG_CAN_ISOTP_ENABLE_CONTEXT_BUFFERS
K_MEM_SLAB_DEFINE(ctx_slab, sizeof(struct iso_tp_send_ctx),
		  CONFIG_CAN_ISOTP_TX_CONTEXT_BUF_COUNT, 4);
#endif

static void receive_pool_free(struct net_buf *buf);
static void receive_ff_sf_pool_free(struct net_buf *buf);

NET_BUF_POOL_DEFINE(isotp_rx_pool, CONFIG_CAN_ISOTP_RX_BUF_COUNT,
		    CONFIG_CAN_ISOTP_RX_BUF_SIZE, sizeof(u32_t),
		    receive_pool_free);

NET_BUF_POOL_DEFINE(isotp_rx_sf_ff_pool, CONFIG_CAN_ISOTP_RX_SF_FF_BUF_COUNT,
		    ISO_TP_CAN_DL, sizeof(u32_t), receive_ff_sf_pool_free);

static struct iso_tp_global_ctx global_ctx = {
	.alloc_list = SYS_SLIST_STATIC_INIT(&global_ctx.alloc_list),
	.ff_sf_alloc_list = SYS_SLIST_STATIC_INIT(&global_ctx.ff_sf_alloc_list)
};

#ifdef CONFIG_CAN_ISOTP_USE_TX_BUF
NET_BUF_POOL_VAR_DEFINE(isotp_tx_pool, CONFIG_CAN_ISOTP_TX_BUF_COUNT,
			CONFIG_CAN_ISOTP_BUF_TX_DATA_POOL_SIZE, NULL);
#endif

K_THREAD_STACK_DEFINE(tx_stack, 512);
static struct k_work_q iso_tp_workq;

static void receive_state_machine(struct iso_tp_rcv_ctx *ctx);
...
static void receive_pool_free(struct net_buf *buf)
{
	struct iso_tp_rcv_ctx *ctx;
	sys_snode_t *ctx_node;

	net_buf_destroy(buf);

	SYS_SLIST_FOR_EACH_NODE(&global_ctx.alloc_list, ctx_node) {
		ctx = CONTAINER_OF(ctx_node, struct iso_tp_rcv_ctx, alloc_node);
...
static inline void receive_report_error(struct iso_tp_rcv_ctx *ctx, int err)
{
	ctx->state = ISO_TP_RX_STATE_ERR;
	ctx->error_nr = err;
}

void receive_can_tx_isr(u32_t err_flags, void *arg)
{
	struct iso_tp_rcv_ctx *ctx = (struct iso_tp_rcv_ctx *)arg;

	if (err_flags) {
		LOG_ERR("Error sending FC frame (%d)", err_flags);
		receive_report_error(ctx, ISO_TP_N_ERROR);
		k_work_submit(&ctx->work);
	}
}
...
static inline u32_t receive_get_ff_length(struct net_buf *buf)
{
	...
	len = ((pci & ISO_TP_PCI_FF_DL_UPPER_MASK) << 8) | net_buf_pull_u8(buf);
...
static inline u32_t receive_get_sf_length(struct net_buf *buf)
{
	...
	return pci & ISO_TP_PCI_SF_DL_MASK;
}

static void receive_send_fc(struct iso_tp_rcv_ctx *ctx, u8_t fs)
{
	...
	__ASSERT_NO_MSG(!(fs & ISO_TP_PCI_TYPE_MASK));

	if (ctx->tx_addr.use_ext_addr) {
		*data++ = ctx->tx_addr.ext_addr;
	}

	*data++ = ISO_TP_PCI_TYPE_FC | fs;
	*data++ = ctx->opts.bs;
	*data++ = ctx->opts.stmin;
	frame.dlc = data - frame.data;

	ret = can_send(ctx->can_dev, &frame, ISO_TP_A, receive_can_tx_isr, ctx);
	if (ret) {
		LOG_ERR("Can't send FC, (%d)", ret);
		receive_report_error(ctx, ISO_TP_N_TIMEOUT_A);
...
static inline struct net_buf *receive_alloc_buffer_chain(u32_t len)
{
	...
	if (len < CONFIG_CAN_ISOTP_RX_BUF_SIZE) {
		return buf;
	}

	remaining_len = len - CONFIG_CAN_ISOTP_RX_BUF_SIZE;
	last = buf;
	while (remaining_len) {
		...
		last = frag;
		remaining_len = remaining_len > CONFIG_CAN_ISOTP_RX_BUF_SIZE ?
				remaining_len - CONFIG_CAN_ISOTP_RX_BUF_SIZE : 0;
	}

	return buf;
}

static int receive_alloc_buffer(struct iso_tp_rcv_ctx *ctx)
{
	...
	if (ctx->opts.bs == 0) {
		...
		buf = receive_alloc_buffer_chain(ctx->opts.bs * (ISO_TP_CAN_DL - 1));
	}

	if (!buf) {
		k_timer_start(&ctx->timeout, ISO_TP_ALLOC_TIMEOUT, 0);

		if (ctx->wft == ISO_TP_WFT_FIRST) {
			LOG_DBG("Allocation failed. Append to alloc list");
			ctx->wft = 0;
			sys_slist_append(&global_ctx.alloc_list, &ctx->alloc_node);
		} else {
			LOG_DBG("Allocation failed. Send WAIT frame");
			ctx->state = ISO_TP_RX_STATE_SEND_WAIT;
			receive_state_machine(ctx);
		}
		return -1;
	}

	if (ctx->state == ISO_TP_RX_STATE_TRY_ALLOC) {
		k_timer_stop(&ctx->timeout);
		ctx->wft = ISO_TP_WFT_FIRST;
...
static void receive_state_machine(struct iso_tp_rcv_ctx *ctx)
{
	...
	switch (ctx->state) {
	case ISO_TP_RX_STATE_PROCESS_SF:
		...
		*ud_rem_len = 0;
		...
		ctx->state = ISO_TP_RX_STATE_RECYCLE;
		receive_state_machine(ctx);
		break;
	case ISO_TP_RX_STATE_PROCESS_FF:
		ctx->length = receive_get_ff_length(ctx->buf);
		LOG_DBG("SM process FF. Length: %d", ctx->length);
		ctx->length -= ctx->buf->len;
		if (ctx->opts.bs == 0 &&
		    ctx->length > CONFIG_CAN_ISOTP_RX_BUF_COUNT *
				  CONFIG_CAN_ISOTP_RX_BUF_SIZE) {
			LOG_ERR("Pkt length is %d but buffer has only %d bytes",
				ctx->length,
				CONFIG_CAN_ISOTP_RX_BUF_COUNT *
				CONFIG_CAN_ISOTP_RX_BUF_SIZE);
			receive_report_error(ctx, ISO_TP_N_BUFFER_OVERFLW);
			receive_state_machine(ctx);
			break;
		}
		...
		ctx->wft = ISO_TP_WFT_FIRST;
		ctx->state = ISO_TP_RX_STATE_TRY_ALLOC;
	case ISO_TP_RX_STATE_TRY_ALLOC:
		...
		ctx->state = ISO_TP_RX_STATE_SEND_FC;
	case ISO_TP_RX_STATE_SEND_FC:
		LOG_DBG("SM send CTS FC frame");
		receive_send_fc(ctx, ISO_TP_PCI_FS_CTS);
		k_timer_start(&ctx->timeout, CONFIG_CAN_ISO_TP_CR_TIMEOUT, 0);
		ctx->state = ISO_TP_RX_STATE_WAIT_CF;
		break;

	case ISO_TP_RX_STATE_SEND_WAIT:
		if (++ctx->wft < CONFIG_CAN_ISO_TP_WFTMAX) {
			LOG_DBG("Send wait frame number %d", ctx->wft);
			receive_send_fc(ctx, ISO_TP_PCI_FS_WAIT);
			k_timer_start(&ctx->timeout, ISO_TP_ALLOC_TIMEOUT, 0);
			ctx->state = ISO_TP_RX_STATE_TRY_ALLOC;
			break;
		}
		...
		LOG_ERR("Sent %d wait frames. Giving up to alloc now",
			ctx->wft);
		receive_report_error(ctx, ISO_TP_N_BUFFER_OVERFLW);
	case ISO_TP_RX_STATE_ERR:
		LOG_DBG("SM ERR state. err nr: %d", ctx->error_nr);
		k_timer_stop(&ctx->timeout);
		if (ctx->error_nr == ISO_TP_N_BUFFER_OVERFLW) {
			receive_send_fc(ctx, ISO_TP_PCI_FS_OVFLW);
		}
	...
	case ISO_TP_RX_STATE_RECYCLE:
		...
		sys_slist_find_and_remove(&global_ctx.ff_sf_alloc_list,
					  &ctx->alloc_node);
		ctx->state = ISO_TP_RX_STATE_WAIT_FF_SF;
	case ISO_TP_RX_STATE_UNBOUND:
		break;
	default:
		break;
	}
...
{
	struct iso_tp_rcv_ctx *ctx = CONTAINER_OF(item, struct iso_tp_rcv_ctx,
						  work);
	receive_state_machine(ctx);
}
...
{
	struct iso_tp_rcv_ctx *ctx = CONTAINER_OF(timer, struct iso_tp_rcv_ctx,
						  timeout);
	switch (ctx->state) {
	case ISO_TP_RX_STATE_WAIT_CF:
		LOG_ERR("Timeout while waiting for CF");
		receive_report_error(ctx, ISO_TP_N_TIMEOUT_CR);
		break;
	case ISO_TP_RX_STATE_TRY_ALLOC:
		ctx->state = ISO_TP_RX_STATE_SEND_WAIT;
		break;
...
static void process_ff_sf(struct iso_tp_rcv_ctx *ctx, struct zcan_frame *frame)
{
	int index = 0;
	...
	switch (frame->data[index] & ISO_TP_PCI_TYPE_MASK) {
	case ISO_TP_PCI_TYPE_FF:
		LOG_DBG("Got FF IRQ");
		if (frame->dlc != ISO_TP_CAN_DL) {
			LOG_INF("FF DL does not match. Ignore");
			return;
		}
		ctx->state = ISO_TP_RX_STATE_PROCESS_FF;
		ctx->sn_expected = 1;
		break;
	case ISO_TP_PCI_TYPE_SF:
		LOG_DBG("Got SF IRQ");
		if ((frame->data[index] & ISO_TP_PCI_FF_DL_UPPER_MASK) + index + 1
		    != frame->dlc) {
			LOG_INF("SF DL does not match. Ignore");
			return;
		}
		ctx->state = ISO_TP_RX_STATE_PROCESS_SF;
		break;
	default:
...
static inline void receive_add_mem(struct iso_tp_rcv_ctx *ctx, u8_t *data,
				  size_t len)
{
...
static void process_cf(struct iso_tp_rcv_ctx *ctx, struct zcan_frame *frame)
{
	...
	if ((frame->data[index] & ISO_TP_PCI_TYPE_MASK) != ISO_TP_PCI_TYPE_CF) {
		LOG_DBG("Waiting for CF but got something else (%d)",
			frame->data[index] >> ISO_TP_PCI_TYPE_POS);
		receive_report_error(ctx, ISO_TP_N_UNEXP_PDU);
		return;
	}

	k_timer_start(&ctx->timeout, CONFIG_CAN_ISO_TP_CR_TIMEOUT, 0);

	if ((frame->data[index++] & ISO_TP_PCI_SN_MASK) != ctx->sn_expected++) {
		LOG_ERR("Sequence number missmatch");
		receive_report_error(ctx, ISO_TP_N_WRONG_SN);
		return;
	}
...
static void receive_can_rx_isr(struct zcan_frame *frame, void *arg)
{
	...
	switch (ctx->state) {
	case ISO_TP_RX_STATE_WAIT_FF_SF:
	...
	case ISO_TP_RX_STATE_WAIT_CF:
		...
		if (ctx->state == ISO_TP_RX_STATE_WAIT_CF) {
			return;
		}
...
static inline int attach_ff_filter(struct iso_tp_rcv_ctx *ctx)
{
		...
		s32_t timeout)
{
	...
	__ASSERT(opts->stmin < ISO_TP_STMIN_MAX, "STmin limit");
	__ASSERT(opts->stmin <= ISO_TP_STMIN_MS_MAX ||
		 opts->stmin >= ISO_TP_STMIN_US_BEGIN, "STmin reserved");
	...
	k_timer_init(&ctx->timeout, receive_timeout_handler, NULL);
...
void iso_tp_unbind(struct iso_tp_rcv_ctx *ctx)
{
	...
	ctx->state = ISO_TP_RX_STATE_UNBOUND;
		   ...
		   s32_t timeout)
{
	...
	if (!buf) {
		ret = ctx->error_nr ? ctx->error_nr : ISO_TP_RCV_TIMEOUT;
		ctx->error_nr = 0;
...
int iso_tp_rcv(struct iso_tp_rcv_ctx *ctx, u8_t *data, size_t len, s32_t timeout)
{
	...
	if (ret) {
		...
		if (ret == -EAGAIN) {
			return ISO_TP_RCV_TIMEOUT;
		}

		return ISO_TP_N_ERROR;
	}
...
static inline void send_report_error(struct iso_tp_send_ctx *ctx, u32_t err)
{
	ctx->state = ISO_TP_TX_ERR;
	ctx->error_nr = err;
}

static void send_can_tx_isr(u32_t err_flags, void *arg)
{
	struct iso_tp_send_ctx *ctx = (struct iso_tp_send_ctx *)arg;

	k_work_submit(&ctx->work);
}

static void send_process_fc(struct iso_tp_send_ctx *ctx, struct zcan_frame *frame)
{
	u8_t *data = frame->data;
	...
	if ((*data & ISO_TP_PCI_TYPE_MASK) != ISO_TP_PCI_TYPE_FC) {
		LOG_ERR("Got unexpected PDU expected FC");
		send_report_error(ctx, ISO_TP_N_UNEXP_PDU);
		return;
	}

	switch (*data++ & ISO_TP_PCI_FS_MASK) {
	case ISO_TP_PCI_FS_CTS:
		ctx->state = ISO_TP_TX_SEND_CF;
		ctx->wft = 0;
		ctx->opts.bs = *data++;
		ctx->opts.stmin = *data++;
		ctx->bs = ctx->opts.bs;
		LOG_DBG("Got CTS. BS: %d, STmin: %d", ctx->opts.bs,
			ctx->opts.stmin);
		break;
	case ISO_TP_PCI_FS_WAIT:
		LOG_DBG("Got WAIT frame");
		k_timer_start(&ctx->timeout, ISO_TP_BS, 0);
		if (ctx->wft >= CONFIG_CAN_ISO_TP_WFTMAX) {
			LOG_INF("Got to many wait frames");
			send_report_error(ctx, ISO_TP_N_WFT_OVRN);
		}
		ctx->wft++;
		break;
	case ISO_TP_PCI_FS_OVFLW:
		LOG_ERR("Got overflow FC frame");
		send_report_error(ctx, ISO_TP_N_BUFFER_OVERFLW);
		break;
	default:
		send_report_error(ctx, ISO_TP_N_INVALID_FS);
...
static void send_can_rx_isr(struct zcan_frame *frame, void *arg)
{
	...
	if (ctx->state == ISO_TP_TX_WAIT_FC) {
...
static size_t get_ctx_data_length(struct iso_tp_send_ctx *ctx)
{
	return ctx->is_net_buf ? net_buf_frags_len(ctx->buf) : ctx->len;
}

static const u8_t *pull_data_ctx(struct iso_tp_send_ctx *ctx, size_t len)
{
	const u8_t *data;

	if (ctx->is_net_buf) {
		data = (u8_t *)net_buf_pull_mem(ctx->buf, len);
	} else {
		data = ctx->data;
		ctx->data += len;
		ctx->len -= len;
...
static inline int send_sf(struct iso_tp_send_ctx *ctx)
{
	struct zcan_frame frame = {
		.id_type  = ctx->tx_addr.id_type,
		.rtr = CAN_DATAFRAME,
	...
	data = pull_data_ctx(ctx, len);

	if (ctx->tx_addr.use_ext_addr) {
		frame.data[index++] = ctx->tx_addr.ext_addr;
	}

	frame.data[index++] = ISO_TP_PCI_TYPE_SF | len;

	__ASSERT_NO_MSG(len <= ISO_TP_CAN_DL - index);
	memcpy(&frame.data[index], data, len);

	frame.dlc = len + index;

	ctx->state = ISO_TP_TX_SEND_SF;
	ret = can_send(ctx->can_dev, &frame, ISO_TP_A, send_can_tx_isr, ctx);
	return ret;
}

static inline int send_ff(struct iso_tp_send_ctx *ctx)
{
	struct zcan_frame frame = {
		.id_type  = ctx->tx_addr.id_type,
		.rtr = CAN_DATAFRAME,
	...
	if (len > 0xFFF) {
		frame.data[index++] = ISO_TP_PCI_TYPE_FF;
		frame.data[index++] = 0;
		...
		frame.data[index++] = ISO_TP_PCI_TYPE_FF | (len >> 8);
		frame.data[index++] = len & 0xFF;
	}
	 ...
	 */
	ctx->sn = 1;
	data = pull_data_ctx(ctx, ISO_TP_CAN_DL - index);
	memcpy(&frame.data[index], data, ISO_TP_CAN_DL - index);

	ret = can_send(ctx->can_dev, &frame, ISO_TP_A, send_can_tx_isr, ctx);
	return ret;
}

static inline int send_cf(struct iso_tp_send_ctx *ctx)
{
	struct zcan_frame frame = {
		.id_type  = ctx->tx_addr.id_type,
		.rtr = CAN_DATAFRAME,
	...
	frame.data[index++] = ISO_TP_PCI_TYPE_CF | ctx->sn++;

	rem_len = get_ctx_data_length(ctx);
	len = MIN(rem_len, ISO_TP_CAN_DL - index);
	rem_len -= len;
	frame.dlc = len + index;
	data = pull_data_ctx(ctx, len);
	memcpy(&frame.data[index], data, len);
...
#ifdef CONFIG_CAN_ISOTP_ENABLE_CONTEXT_BUFFERS
static inline void free_send_ctx(struct iso_tp_send_ctx **ctx)
{
	if ((*ctx)->is_net_buf) {
		net_buf_unref((*ctx)->buf);
		(*ctx)->buf = NULL;
...
static int alloc_ctx(struct iso_tp_send_ctx **ctx, s32_t timeout)
{
	int ret;

	ret = k_mem_slab_alloc(&ctx_slab, (void **)ctx, timeout);
	if (ret) {
		return ISO_TP_NO_CTX_LEFT;
	}
...
#endif /*CONFIG_CAN_ISOTP_ENABLE_CONTEXT_BUFFERS*/

static s32_t stmin_to_time(u8_t stmin)
{
	/* According to ISO 15765-2 stmin should be 127ms if value is corrupt */
	if (stmin > ISO_TP_STMIN_MAX ||
	    (stmin > ISO_TP_STMIN_MS_MAX && stmin < ISO_TP_STMIN_US_BEGIN)) {
		return K_MSEC(ISO_TP_STMIN_MS_MAX);
	}

	/* This should be 100us-900us but zephyr can't handle that*/
	if (stmin >= ISO_TP_STMIN_US_BEGIN) {
		return K_MSEC(1);
	}
...
static void send_state_machine(struct iso_tp_send_ctx *ctx)
{
	int ret;

	switch (ctx->state) {

	case ISO_TP_TX_SEND_FF:
		LOG_DBG("SM send FF");
		send_ff(ctx);
		k_timer_start(&ctx->timeout, ISO_TP_BS, 0);
		ctx->state = ISO_TP_TX_WAIT_FC;
		break;

	case ISO_TP_TX_SEND_CF:
		LOG_DBG("SM send CF");
		ret = send_cf(ctx);
		if (!ret) {
			k_timer_stop(&ctx->timeout);
			ctx->state = ISO_TP_TX_WAIT_FIN;
			break;
		}
		...
		if (ctx->opts.bs && !--ctx->bs) {
			...
			ctx->state = ISO_TP_TX_WAIT_ST;
		}

		k_timer_start(&ctx->timeout, ISO_TP_BS, 0);
		break;

	case ISO_TP_TX_WAIT_ST:
		LOG_DBG("SM wait ST");
		k_timer_start(&ctx->timeout, stmin_to_time(ctx->opts.stmin), 0);
		ctx->state = ISO_TP_TX_SEND_CF;
		break;

	case ISO_TP_TX_ERR:
		LOG_DBG("SM error");
	case ISO_TP_TX_WAIT_FIN:
		if (ctx->filter_id >= 0) {
			can_detach(ctx->can_dev, ctx->filter_id);
		}
		...
		ctx->state = ISO_TP_TX_STATE_RESET;
		break;
...
{
	struct iso_tp_send_ctx *ctx = CONTAINER_OF(item, struct iso_tp_send_ctx,
						   work);
...
{
	struct iso_tp_send_ctx *ctx = CONTAINER_OF(timer, struct iso_tp_send_ctx,
						   timeout);
	if (ctx->state != ISO_TP_TX_SEND_CF) {
		send_report_error(ctx, ISO_TP_N_TIMEOUT_BS);
...
static inline int attach_fc_filter(struct iso_tp_send_ctx *ctx)
{
	struct zcan_filter filter = {
		.id_type = ctx->rx_addr.id_type,
		.rtr = CAN_DATAFRAME,
...
static int send(struct iso_tp_send_ctx *ctx, struct device *can_dev,
		const struct iso_tp_msg_id *tx_addr,
		const struct iso_tp_msg_id *rx_addr,
	...
	if (len > ISO_TP_CAN_DL - (tx_addr->use_ext_addr ? 2 : 1)) {
		...
		if (ret) {
			return ret == CAN_TIMEOUT ?
				      ISO_TP_N_TIMEOUT_A : ISO_TP_N_ERROR;
		}
	}
		    ...
		    s32_t timeout)
{
	...
	if (!buf) {
		k_mem_slab_free(&ctx_slab, (void **)&ctx);
		return ISO_TP_NO_BUF_DATA_LEFT;
	}
...
#endif /*CONFIG_CAN_ISOTP_USE_TX_BUF*/
#endif /*CONFIG_CAN_ISOTP_ENABLE_CONTEXT_BUFFERS*/

static int iso_tp_workq_init(struct device *dev)
{
	ARG_UNUSED(dev);
	LOG_DBG("Starting workqueue");
	k_work_q_start(&iso_tp_workq,
		       tx_stack,
		       K_THREAD_STACK_SIZEOF(tx_stack),
		       CONFIG_CAN_ISOTP_WORKQUEUE_PRIO);
	k_thread_name_set(&iso_tp_workq.thread, "isotp_work");

	return 0;
}

SYS_INIT(iso_tp_workq_init, POST_KERNEL, CONFIG_APPLICATION_INIT_PRIORITY);
