...
static void net_can_rcv(struct zcan_frame *frame, void *arg)
{
	...
	pkt = net_pkt_rx_alloc_with_buffer(ctx->iface, sizeof(struct zcan_frame),
					   AF_INET6, 0, K_NO_WAIT);
	if (!pkt) {
		LOG_ERR("Failed to obtain net_pkt with size of %d",
			sizeof(struct zcan_frame));
		goto drop;
	}

	pkt->can_rx_ctx = NULL;
	ret = net_pkt_write(pkt, frame, sizeof(struct zcan_frame));
	if (ret) {
...
static inline int attach_ff_filter(struct net_can_context *ctx)
{
...
static int enable(struct device *dev, bool enable)
{
	...
	if (enable) {
		if (ctx->rcv_filter_id == CAN_NET_FILTER_NOT_SET) {
			ctx->rcv_filter_id = attach_ff_filter(ctx);
