...
struct frame_desired des_frames[CEIL((DATA_SEND_LENGTH - DATA_SIZE_FF), DATA_SIZE_CF)];
...
const struct iso_tp_msg_id rx_addr_ext = {
	.std_id = 0x10,
	.id_type = CAN_STANDARD_IDENTIFIER,
	.use_ext_addr = 1,
	.ext_addr = EXT_ADDR
...
const struct iso_tp_msg_id tx_addr_ext = {
	.std_id = 0x11,
	.id_type = CAN_STANDARD_IDENTIFIER,
	.use_ext_addr = 1,
	.ext_addr = EXT_ADDR
...
static void send_sf(void)
{
	...
	ret = iso_tp_send(&send_ctx, can_dev, random_data, DATA_SIZE_SF,
			  &rx_addr, &tx_addr, send_complette_cb, ISO_TP_N_OK);
	zassert_equal(ret, 0, "Send returned %d", ret);
}


static void get_sf(struct iso_tp_rcv_ctx *rcv_ctx, size_t data_size)
{
	int ret;

	memset(data_buf, 0, sizeof(data_buf));
	ret = iso_tp_rcv(rcv_ctx, data_buf, sizeof(data_buf), K_MSEC(1000));
	zassert_equal(ret, data_size, "rcv returned %d", ret);

	ret = check_data(data_buf, random_data, data_size);
	zassert_equal(ret, 0, "Data differ");
}

static void get_sf_ignore(struct iso_tp_rcv_ctx *rcv_ctx)
{
	int ret;

	ret = iso_tp_rcv(rcv_ctx, data_buf, sizeof(data_buf), K_MSEC(200));
	zassert_equal(ret, ISO_TP_RCV_TIMEOUT, "rcv returned %d", ret);
}
			      ...
			      size_t len, s32_t delay)
{
	...
	do {
		memset(data_buf, 0, sizeof(data_buf));
		rcv_len = iso_tp_rcv(rcv_ctx, data_buf, sizeof(data_buf),
				 K_MSEC(1000));
		zassert_true(rcv_len >= 0, "rcv error: %d", rcv_len);

		zassert_true(remaining_len >= rcv_len, "More data then expected");
		ret = check_data(data_buf, data_ptr, rcv_len);
		zassert_equal(ret, 0, "Data differ");
		data_ptr += rcv_len;
		remaining_len -= rcv_len;
...
static void test_receive_sf(void)
{
	...
	zassert_equal(ret, ISO_TP_N_OK, "Binding failed [%d]", ret);

	send_frame_series(&single_frame, 1, rx_addr.std_id);

	get_sf(&rcv_ctx, DATA_SIZE_SF);
...
static void test_send_sf_ext(void)
{
	...
	ret = iso_tp_send(&send_ctx, can_dev, random_data, DATA_SIZE_SF_EXT,
			  &rx_addr_ext, &tx_addr_ext, send_complette_cb, ISO_TP_N_OK);
	zassert_equal(ret, 0, "Send returned %d", ret);
...
static void test_receive_sf_ext(void)
{
	...
	ret = iso_tp_bind(&rcv_ctx, can_dev, &rx_addr_ext, &tx_addr,
			  &fc_opts_single, K_NO_WAIT);
	zassert_equal(ret, ISO_TP_N_OK, "Binding failed [%d]", ret);

	send_frame_series(&single_frame, 1, rx_addr.std_id);

	get_sf(&rcv_ctx, DATA_SIZE_SF_EXT);
...
static void test_receive_data(void)
{
	...
	receive_test_data(&rcv_ctx, random_data, DATA_SEND_LENGTH, 0);
...
static void test_send_timeouts(void)
{
	...
	ret = iso_tp_send(&send_ctx, can_dev, random_data, sizeof(random_data),
			 &tx_addr, &rx_addr, NULL, NULL);
	time_diff = k_uptime_get_32() - start_time;
	zassert_equal(ret, ISO_TP_N_TIMEOUT_BS, "Expected timeout but got %d",
		      ret);
...
static void test_receive_timeouts(void)
{
	...
	ret = iso_tp_rcv(&rcv_ctx, data_buf, sizeof(data_buf), K_FOREVER);
	zassert_equal(ret, DATA_SIZE_FF,
		      "Expected FF data length but got %d", ret);
	ret = iso_tp_rcv(&rcv_ctx, data_buf, sizeof(data_buf), K_FOREVER);
	zassert_equal(ret, ISO_TP_N_TIMEOUT_CR,
		      "Expected timeout but got %d", ret);
...
void test_receiver_fc_errors(void)
{
	...
	zassert_equal(ret, DATA_SIZE_FF,
		      "Expected FF data length but got %d", ret);
	ret = iso_tp_rcv(&rcv_ctx, data_buf, sizeof(data_buf), K_MSEC(200));
	zassert_equal(ret, ISO_TP_N_WRONG_SN,
...
void test_sender_fc_errors(void)
{
	...
	k_sem_reset(&send_compl_sem);
	ret = iso_tp_send(&send_ctx, can_dev, random_data, DATA_SEND_LENGTH,
			  &tx_addr, &rx_addr, send_complette_cb,
			  (void *)ISO_TP_N_INVALID_FS);
	...
	/* buffer overflow */
	can_detach(can_dev, filter_id);
	ret = iso_tp_bind(&rcv_ctx, can_dev, &tx_addr, &rx_addr,
			  &fc_opts_single, K_NO_WAIT);
	zassert_equal(ret, ISO_TP_N_OK, "Binding failed [%d]", ret);

	ret = iso_tp_send(&send_ctx, can_dev, random_data, 5*1024,
			  &tx_addr, &rx_addr, NULL, NULL);
	zassert_equal(ret, ISO_TP_N_BUFFER_OVERFLW,
	...
	ret = iso_tp_send(&send_ctx, can_dev, random_data, DATA_SEND_LENGTH,
			  &tx_addr, &rx_addr, send_complette_cb,
			  (void *)ISO_TP_N_WFT_OVRN);

	check_frame_series(&ff_frame, 1, &frame_msgq);
	fc_frame.data[0] = FC_PCI_BYTE_1(FC_PCI_WAIT);
	for (i = 0; i < CONFIG_CAN_ISO_TP_WFTMAX + 1; i++) {
		send_frame_series(&fc_frame, 1, rx_addr.std_id);
	}
