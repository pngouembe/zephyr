...
struct zcan_frame {
	...
	union {
		u8_t data[CAN_MAX_DLEN];
		u32_t data_32[CAN_MAX_DLEN / sizeof(u32_t)];
	};
...
{
	static const u8_t dlc_table[] = {1, 2, 3, 4, 5, 6, 7, 8, 12,
					 16, 20, 24, 32, 48, 64};
 ...
 * @param fd_format    Send the frame in CAN-FD frame format. Allows dlc > 8.
				     ...
				     void *callback_arg)
{
	...
	return api->send(dev, frame, timeout, brs, fd_format, callback_isr,
			 callback_arg);
}
#endif /* CONFIG_CAN_FD_MODE */
